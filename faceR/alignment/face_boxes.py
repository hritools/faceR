...
def align_faces(frames, conf):
    detector = FaceDetector(conf['model'])

    for frame in frames:
        # Image.fromarray(frame).save('face.jpg')
        dets = detector.__call__(frame)
        # print(dets)

        faces = [[face[1], face[0], face[3], face[2]] for face in dets[0].astype(int)]
        yield frame, faces

    logging.debug('exiting face-boxes align')
