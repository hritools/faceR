...
ARG DLDT_BRANCH=2018_R5
RUN git clone --depth=1 -b ${DLDT_BRANCH} https://github.com/opencv/dldt.git ${DLDT_DIR} && \
...
RUN curl -L -o ${DLDT_DIR}/mklml_lnx_2019.0.1.20180928.tgz https://github.com/intel/mkl-dnn/releases/download/v0.17.2/mklml_lnx_2019.0.1.20180928.tgz && \
    tar -xzf ${DLDT_DIR}/mklml_lnx_2019.0.1.20180928.tgz && \
    rm ${DLDT_DIR}/mklml_lnx_2019.0.1.20180928.tgz
WORKDIR ${DLDT_DIR}/inference-engine
RUN mkdir build && cd build && cmake -DGEMM=MKL \
    -DMKLROOT=${DLDT_DIR}/mklml_lnx_2019.0.1.20180928 \
    -DENABLE_MKL_DNN=ON  \
    -DCMAKE_BUILD_TYPE=Release ..
RUN cd build && make -j4
RUN pip3 install cython numpy && mkdir ie_bridges/python/build && cd ie_bridges/python/build && \
    cmake \
    -DInferenceEngine_DIR=${DLDT_DIR}/inference-engine/build -DPYTHON_EXECUTABLE=`which python3` \
